...
def _run_relay(action: str, **kwargs) -> Optional[dict]:
    """
    Helper: menjalankan relay_runner.ts via subprocess dan mem-parse output JSON-nya.

    Args:
        action : 'anchor' atau 'verify'
        **kwargs: payload atau uuid, tergantung action.

    Returns:
        Dict dari JSON output relay, atau None jika gagal.
    """
    ...
    try:
        result = subprocess.run(
            ...
            text=True,
            cwd=str(_RELAY_DIR),
            timeout=120,  # 120 detik; blockchain tx bisa memakan waktu saat Amoy padat
            env={**os.environ},
        )

        # Relay mencetak JSON di baris terakhir stdout
        ...
        if result.returncode != 0:
            logger.warning(
                f"[WEB3] Relay exited {result.returncode}. "
                f"Parsed: {parsed}. STDERR: {result.stderr[:300]}"
            )
        else:
...
def log_incident_to_blockchain(payload: dict) -> dict:
    """
    Mencatat insiden ke Polygon Amoy via relay.ts.

    Mendukung mode PRIMARY (SipartaAudit) dan LEGACY (GasDetectionStorage).
    Untuk mode LEGACY, payload harus mengandung field sensor (mics5524, tgs2600, mq2, mq135).

    Args:
        payload: Dict dengan field:
            - incident_id  : UUID insiden (dari Supabase)
            - classification: 'aman' | 'waspada' | 'bahaya'
            - mics5524     : nilai sensor (float)
            - tgs2600      : nilai sensor (float)
            - mq2          : nilai sensor (float)
            - mq135        : nilai sensor (float)
            - image_url    : URL foto bukti (opsional)
            - ipfs_cid     : CID IPFS metadata (opsional, ada default di relay_runner.ts)

    Returns:
        Dict dengan key: txHash, blockNumber, mode
    """
    result = _run_relay("anchor", payload=payload)
    if result is None:
        return {"txHash": None, "blockNumber": 0, "mode": "error"}
    return result


def verify_incident_on_chain(incident_uuid: str) -> bool:
    """
    Verifikasi apakah insiden sudah tercatat di SipartaAudit contract.
    Hanya berfungsi di mode PRIMARY. Mengembalikan False di mode LEGACY.

    Args:
        incident_uuid: UUID insiden (string, dari PostgreSQL).

    Returns:
        True jika insiden terverifikasi on-chain, False jika tidak.
    """
    result = _run_relay("verify", uuid=incident_uuid)
    if result is None:
        return False
